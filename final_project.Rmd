...
training_set$sum_scaled <- scale(training_set$sum, center = mean_training, scale = sd_training)
validation_set$sum_scaled <- scale(validation_set$sum, center = mean_training, scale = sd_training)

# Turn into a time series format & scale the sum column for training set
ts_training_set <- ts(scale(training_set$sum, center = mean_training, scale = sd_training),
                      start = train_start_date, end = train_end_date, frequency = 365)

# Scale the validation set using the mean and standard deviation from the training set
ts_validation_set <- ts(scale(validation_set$sum, center = mean_training, scale = sd_training),
                        start = test_start_date, end = test_end_date, frequency = 365)
...
# Fit an ARIMA model
arima_model <- auto.arima(ts_training_set)

# Display the model summary
summary(arima_model)

# Make predictions on the validation set
forecast_values <- forecast(arima_model, h = length(ts_validation_set))

# Invert the scaling to get predictions in the original scale
forecast_values <- forecast_values$mean * sd_training + mean_training

# Plot the actual vs predicted values for the validation set
plot(ts_validation_set, col = "blue", main = "ARIMA Model - Validation Set", ylab = "sum", xlab = "Date")
...
neural_model <- neuralnet(sum_scaled ~ DATE, data = training_set, hidden = c(5, 3), linear.output = TRUE)

# Make predictions on the training set
training_predictions <- predict(neural_model, training_set)

# Make predictions on the validation set
validation_predictions <- predict(neural_model, validation_set)

# Include DATE in the prediction data frames
df_training_predictions <- data.frame(DATE = training_set$DATE, sum_scaled = training_predictions)
df_validation_predictions <- data.frame(DATE = validation_set$DATE, sum_scaled = validation_predictions)

# Calculate RMSE for training set
rmse_training <- sqrt(mean((training_set$sum_scaled - df_training_predictions$sum_scaled)^2))

# Calculate RMSE for validation set
rmse_validation <- sqrt(mean((validation_set$sum_scaled - df_validation_predictions$sum_scaled)^2))

# Print RMSE values
cat("RMSE for Training Set:", rmse_training, "\n")
cat("RMSE for Validation Set:", rmse_validation, "\n")

# Plot actual vs predicted for training set
ggplot() +
  geom_line(data = training_set, aes(x = DATE, y = sum_scaled), color = "blue", size = 1, linetype = "solid", alpha = 0.7) +
  geom_line(data = df_training_predictions, aes(x = DATE, y = sum_scaled), color = "red", size = 1, linetype = "dashed", alpha = 0.7) +
  labs(title = "Neural Network Model - Training Set", x = "Date", y = "sum") +
  theme_minimal()

# Plot actual vs predicted for validation set
ggplot() +
  geom_line(data = validation_set, aes(x = DATE, y = sum_scaled), color = "blue", size = 1, linetype = "solid", alpha = 0.7) +
  geom_line(data = df_validation_predictions, aes(x = DATE, y = sum_scaled), color = "red", size = 1, linetype = "dashed", alpha = 0.7) +
  labs(title = "Neural Network Model - Validation Set", x = "Date", y = "sum") +
  theme_minimal()
```

```{r}
# Convert the training_set and validation_set to time series
ts_training_set <- ts(training_set$sum_scaled, start = train_start_date, end = train_end_date, frequency = 365)
ts_validation_set <- ts(validation_set$sum_scaled, start = test_start_date, end = test_end_date, frequency = 365)

# Create lagged values
lagged_training_set <- embed(ts_training_set, 2)[, 2:1]
colnames(lagged_training_set) <- c("lag1", "sum_scaled")

lagged_validation_set <- embed(ts_validation_set, 2)[, 2:1]
colnames(lagged_validation_set) <- c("lag1", "sum_scaled")

# Create a neural network model
neural_model <- neuralnet(sum_scaled ~ lag1, data = lagged_training_set, hidden = c(5, 3), linear.output = TRUE)

# Make predictions on the training set
training_predictions <- predict(neural_model, lagged_training_set)

# Make predictions on the validation set
validation_predictions <- predict(neural_model, lagged_validation_set)

# Include DATE in the prediction data frames
df_training_predictions <- data.frame(DATE = index(ts_training_set)[-1], sum_scaled = training_predictions)
df_validation_predictions <- data.frame(DATE = index(ts_validation_set)[-1], sum_scaled = validation_predictions)

# Calculate RMSE for training set
rmse_training <- sqrt(mean((lagged_training_set$sum_scaled - df_training_predictions$sum_scaled)^2))

# Calculate RMSE for validation set
rmse_validation <- sqrt(mean((lagged_validation_set$sum_scaled - df_validation_predictions$sum_scaled)^2))
...
# Plot actual vs predicted for training set
ggplot() +
  geom_line(data = lagged_training_set, aes(x = index(ts_training_set)[-1], y = sum_scaled), color = "blue", size = 1, linetype = "solid", alpha = 0.7) +
  geom_line(data = df_training_predictions, aes(x = DATE, y = sum_scaled), color = "red", size = 1, linetype = "dashed", alpha = 0.7) +
  labs(title = "Neural Network Model - Training Set", x = "Date", y = "sum") +
  theme_minimal()
...
# Plot actual vs predicted for validation set
ggplot() +
  geom_line(data = lagged_validation_set, aes(x = index(ts_validation_set)[-1], y = sum_scaled), color = "blue", size = 1, linetype = "solid", alpha = 0.7) +
  geom_line(data = df_validation_predictions, aes(x = DATE, y = sum_scaled), color = "red", size = 1, linetype = "dashed", alpha = 0.7) +
  labs(title = "Neural Network Model - Validation Set", x = "Date", y = "sum") +
  theme_minimal()
```
...
# Convert variables to numeric
training_set$DATE <- as.numeric(training_set$DATE)
training_set$sum <- as.numeric(training_set$sum)
validation_set$DATE <- as.numeric(validation_set$DATE)
validation_set$sum <- as.numeric(validation_set$sum)

# Build a neural network model
neural_model <- neuralnet(sum ~ DATE, data = training_set, hidden = c(5, 3))

# Make predictions on the validation data
validation_set$predicted_qty_neural <- predict(neural_model, newdata = validation_set)

# Plot actual vs. predicted for the neural network
ggplot(validation_set, aes(x = DATE, y = sum)) +
  geom_line(aes(y = predicted_qty_neural), color = "red") +
  labs(title = "Neural Network Model - Actual vs. Predicted sum",
       x = "Date",
       y = "sum") +
  theme_minimal()

# Calculate Root Mean Squared Error (RMSE) for the neural network
rmse_neural <- sqrt(mean((validation_set$sum - validation_set$predicted_qty_neural)^2))
