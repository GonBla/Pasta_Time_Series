...
```{r}
# Fit an ARIMA model to training subset
arima_model_B1_above <- auto.arima(B1_above_ts_entire)
arima_model_B1_below <- auto.arima(B1_below_ts_entire)

# Print model summary
summary(arima_model_B1_above) 
summary(arima_model_B1_below) 

# Generate forecasts using the ARIMA model
forecast_values_above <- forecast(arima_model_B1_above, h = length(B1_above_ts_entire))
forecast_values_below <- forecast(arima_model_B1_below, h = length(B1_below_ts_entire))

# Create a data frame with actual vs predicted vs difference values
results_above_df <- data.frame(
  Date = B1_above$DATE,
  Actual = B1_above_ts_entire,
  Predicted = forecast_values_above$mean
)
results_below_df <- data.frame(
  Date = B1_below$DATE,
  Actual = B1_below_ts_entire,
  Predicted = forecast_values_below$mean
)

# Display the results
forecast_values_above
forecast_values_below
```

```{r}
# Plot above results
ggplot(results_above_df, aes(x = Date)) +
  geom_line(aes(y = Actual, color = "Actual"), linewidth = 1) +
  geom_line(aes(y = Predicted, color = "Predicted"), linewidth = 1) +
  labs(title = "B1 Above- Actual vs Predicted Time Series",
       x = "Date",
...
# Plot above results
ggplot(results_below_df, aes(x = Date)) +
  geom_line(aes(y = Actual, color = "Actual"), linewidth = 1) +
  geom_line(aes(y = Predicted, color = "Predicted"), linewidth = 1) +
  labs(title = "B1 Below- Actual vs Predicted Time Series",
       x = "Date",
